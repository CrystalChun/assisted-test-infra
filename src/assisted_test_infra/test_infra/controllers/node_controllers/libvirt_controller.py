...
class LibvirtController(NodeController, ABC):
    ...
    def _create_dns_host_xml(self, api_vip: str, cluster_name: str):
        """"Creates an XML objet for adding a DNS host to a libvirt network """
    ...
    def add_dns_host_to_network(self, network_name: str, api_vip: str, cluster_name: str):
        ...
        network = self.get_network_by_name(network_name)
        network.update(libvirt.VIR_NETWORK_UPDATE_COMMAND_ADD_LAST, libvirt.VIR_NETWORK_SECTION_DNS_HOST, -1, host_xml.toprettyxml())
    ...
    def remove_dns_host_from_network(self, network_name: str, api_vip: str, cluster_name: str):
        ...
        host_xml = self._create_dns_host_xml(api_vip, cluster_name)
        network = self.get_network_by_name(network_name)
        network.update(libvirt.VIR_NETWORK_UPDATE_COMMAND_DELETE, libvirt.VIR_NETWORK_SECTION_DNS_HOST, -1, host_xml.toprettyxml())
